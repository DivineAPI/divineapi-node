 ...
 */
export function resolveHouseSystem(value: string): string {
  ...
  throw new ValidationError(
    `Invalid house_system '${value}'. Must be one of: ${HOUSE_SYSTEM_FRIENDLY_NAMES} ` +
 ...
 */
export function applyHouseSystem(params: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function applyBirthSecondDefaults(
  params: Record<string, unknown>
): Record<string, unknown> {
  let out = params;
  const fill = (secKey: string, minKey: string): void => {
